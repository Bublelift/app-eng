...
class PuzzleFragment : Fragment() {
    ...
    private fun endGame() {
        var score = 0
        if (requireActivity().findViewById<RadioButton>(binding.quizQuestion1answers.checkedRadioButtonId) != null && requireActivity().findViewById<RadioButton>(binding.quizQuestion1answers.checkedRadioButtonId).text == quizViewModel.correctAnswers.value!![0]) score += 50
        if (requireActivity().findViewById<RadioButton>(binding.quizQuestion2answers.checkedRadioButtonId) != null && requireActivity().findViewById<RadioButton>(binding.quizQuestion2answers.checkedRadioButtonId).text == quizViewModel.correctAnswers.value!![1]) score += 50
        if (requireActivity().findViewById<RadioButton>(binding.quizQuestion3answers.checkedRadioButtonId) != null && requireActivity().findViewById<RadioButton>(binding.quizQuestion3answers.checkedRadioButtonId).text == quizViewModel.correctAnswers.value!![2]) score += 50
