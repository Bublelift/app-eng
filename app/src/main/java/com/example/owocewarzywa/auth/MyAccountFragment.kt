...
class MyAccountFragment : Fragment() {
    ...
    override fun onCreateView( //utożsamienie elementó layoutu z kodem logiki
        ...
        view.apply {
            ...
            findViewById<Button>(R.id.save).setOnClickListener { //walidacja
                if (findViewById<EditText>(R.id.nickname_input).text.toString().length < 1)
                ...
                else { //wywołanie funkcji aktualziujacej dane użytkownika
                    ...
                    else { //jw ale jak nie ma obrazka profilowego
                        FirestoreUtil.updateCurrentUser(
                            findViewById<EditText>(R.id.nickname_input).text.toString(),
                            findViewById<EditText>(R.id.bio_input).text.toString(), null
                        )
                    }
                    Toast.makeText(requireContext(), "Zapisano zmiany", Toast.LENGTH_SHORT).show() //komunikat success
                }
            }
            findViewById<Button>(R.id.signout).setOnClickListener{ //wylogowanie
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) { //renderowanie obrazka
    ...
    override fun onStart() { //zaczytanie danych aktualnie zalogowanego użytkownika
        super.onStart()
